...
b.replace("300x225","1024x768");window.open(b,"","resizeable=yes;toolbar=no;status=no");return false}),0<partID?showPart(partID):0<year.length&&(0<make.length&&0<model.length&&0<style.length)&&(year=decodeURIComponent(year),make=decodeURIComponent(make),model=decodeURIComponent(model),style=decodeURIComponent(style),jQuery.get("https://api.curtmfg.com/v2/GetParts?callback=loadParts",{year:year,make:make,model:model,style:style,cust_id:customer_id,integrated:integrated,dataType:"jsonp"},function(){},
"jsonp")),$.get("https://api.curtmfg.com/v2/AddDeployment?url="+window.location.hostname,function(){},"jsonp"))}function checkVersion(b){return"jquery"==b&&!(parseFloat(jQuery.fn.jquery)<REQUIRED_JQUERY)?!0:!1}function showPart(b){0<b&&jQuery.get("https://api.curtmfg.com/v2/GetPart?callback=loadSingle",{partID:b,dataType:"jsonp",integrated:integrated,cust_id:customer_id},function(){},"jsonp")}
...
function loadParts(b){vehicleStr=mount+" "+year+" "+make+" "+model+" "+style;vehicleStr.replace("rear","Rear Mount").replace("front","Front Mount");mount=year=make=model=style="";if(null!==b&&0<b.length){var c="",b=jQuery(b).sort(sortByClass),a=[],e=[];jQuery.each(b,function(b,c){if(-1==c.shortDesc.toUpperCase().indexOf("T-CONNECTOR",0)||-1==c.shortDesc.toUpperCase().indexOf("WIRING",0))-1==jQuery.inArray(jQuery.trim(c.pClass),a)&&(0<jQuery.trim(c.pClass).length?a.push(jQuery.trim(c.pClass)):-1==
jQuery.inArray("Other",a)&&a.push("Other"))});0<a.length&&(c+='<ul class="tabs">',jQuery.each(a,function(a,f){void 0!==f&&0<f.length&&(e[f]=GetPartsForClass(b,f),c+="<li>",c+='<a href="class'+a+'_content" title="'+f.replace(/ /g,"")+'">'+f+"</a>",c+="</li>")}),c+="</ul>");c+='<div id="widget_tab_container">';jQuery.each(a,function(a,b){c=c+('<div class="widget_tab_content" id="'+b.replace(/ /g,"")+'">');c=c+'<div id="resultBox_outline">';c=c+'<div id="resultBox">';c=c+('<p id="vehicleStr">'+vehicleStr.replace("rear",
"Rear Mount").replace("front","Front Mount")+"</p>");if(logoImg!==""){c=c+'<div id="logo">';c=c+('<img src="'+logoImg+'" alt="Company Logo" />');c=c+"</div>"}c=c+'<div style="clear:both"></div>';c=c+('<span>( <span id="hitchCount">'+e[b].length+"</span> ) products found.</span>");c=c+"</div>";c=c+"</div>";c=c+'<div style="clear:both"></div>';jQuery.each(e[b],function(a,b){if(b.shortDesc.toUpperCase().indexOf("T-CONNECTOR",0)==-1||b.shortDesc.toUpperCase().indexOf("WIRING",0)==-1)c=c+displayPart(b)});
c=c+"</div>"});c+="</div>";jQuery("#hitchResults").html(c);jQuery("#hitchResults").show();jQuery("#loader").before(mountHTML);jQuery("#mount").show();jQuery("#searchStr").text("");displayWiring()}else vehicleStr.replace("rear","Rear Mount").replace("front","Front Mount"),jQuery("#hitchResults").html('<p id="vehicleStr">No results for '+vehicleStr+"</p>");jQuery("#loader").hide();jQuery("#hitchResults").show();jQuery(".widget_tab_content:first").show();jQuery("ul.tabs li:first a").addClass("active")}
...
function GetAccessories(b){jQuery.get("https://api.curtmfg.com/v2/GetRelatedParts?callback=loadAccessories",{partID:b,dataType:"jsonp",widget:!0,integrated:integrated,cust_id:customer_id},function(){},"jsonp")}
...
('<input name="ProductID" id="ProductID" type="hidden" value="'+a+'" />')+'<input name="VariantID" id="VariantID" type="hidden" value="0" /><small>Quantity:</small><input name="Quantity" id="Quantity" type="text" value="1" size="3" maxlength="4" /><input type="submit" value="Add to Cart" /></form>';break;case "fasttrackracks":d=d+'<form action="http://www.fasttrackracks.com/store/addtocart.aspx" method="post">'+('<div style="padding-top:15px"><span class="price accPrice">'+b+"</span>")+'<label>Qty</label><select name="qty" style="min-width:40px;display:inline"><option>1</option><option>2</option><option>3</option><option>4</option><option>5</option></select></div>'+
('<input type="hidden" name="return" value="'+window.location+'" />');d=d+('<input type="hidden" name="imageurl" value="https://www.curtmfg.com/masterlibrary/'+e+"/images/"+e+'_300x225_a.jpg" />')+('<input type="hidden" name="ItemNbr" value="'+c+'" />')+('<input type="hidden" name="product" value="'+vehicleStr+" "+c+'" />');d=d+'<input type="hidden" name="notax" value="null" />'+('<input type="hidden" name="price" value="'+b+'" />')+'<input type="hidden" name="weight" value="null" /><input type="submit" name="submit" class="fasttrackracks_button" value="Buy Now" /></form>';
break;case "stowaway2":d=d+'<form action="http://www.stowaway2.com/store/addtocart.aspx" method="post">'+('<div style="padding-top:15px"><span class="price accPrice">'+b+"</span>")+'<label>Qty</label><select name="qty" style="min-width:40px;display:inline"><option>1</option><option>2</option><option>3</option><option>4</option><option>5</option></select></div>'+('<input type="hidden" name="return" value="'+window.location+'" />');d=d+('<input type="hidden" name="imageurl" value="https://www.curtmfg.com/masterlibrary/'+
e+"/images/"+e+'_300x225_a.jpg" />')+('<input type="hidden" name="ItemNbr" value="'+c+'" />')+('<input type="hidden" name="product" value="'+vehicleStr+" "+c+'" />');d=d+'<input type="hidden" name="notax" value="null" />'+('<input type="hidden" name="price" value="'+b+'" />')+'<input type="hidden" name="weight" value="null" /><input type="submit" name="submit" class="fasttrackracks_button" value="Buy Now" /></form>';break;case "custom":d=0<cart_link.length?d+('<span class="price">'+b+"</span><br />")+
('<a href="'+cart_link.replace("[part_id]",a)+'" title="Buy Now">'):d+('<a href="javascript:void()" title="Buy Now" class="tiretown" id="'+a+'">'),d+='<img src="https://www.curtmfg.com/widget_v2/img/checkout.png" alt="checkout" /></a><br />'}return d};